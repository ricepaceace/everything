...
int main()
{
	...
	if (XGpio_Initialize(&stim, XPAR_STIM_DEVICE_ID) != XST_SUCCESS) {
	  ...
	  */
	 XGpio_DiscreteWrite(&stim, GPIO_CHANNEL, 0x0000);
	...
	xil_printf("starting peak detection demo: gathering data on 1 channel\r\n");
	newms = false;
	int i = 0;
	...
	for(;;)
	{
		if(newms)
		{
			...
			{
				on_v = ms +30;
				XGpio_DiscreteWrite(&stim, GPIO_CHANNEL, 0x1000);
			...
			{
				on_a = ms+30;
				XGpio_DiscreteWrite(&stim, GPIO_CHANNEL, 0x4000);
				xil_printf("-- ATRIA\r\n");
