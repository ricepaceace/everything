...
int main()
{
	...
	for(;;)
	{
		if(newms)
		{
			...
			{
				on_a_peak = ms + 300;
				XGpio_DiscreteWrite(&stim, GPIO_CHANNEL, ATRIAL_PEAK_GPIO);
				for(i = 0; i <3; i++)
					a_peak_votes[i] = 0;
			...
			{
				on_v_peak = ms + 300;
				XGpio_DiscreteWrite(&stim, GPIO_CHANNEL, VENTRICLE_PEAK_GPIO);
				for(i = 0; i <3; i++)
					v_peak_votes[i] = 0;
			}
			if(a_miss_votes[0] + a_miss_votes[1] + a_miss_votes[2] > 1)
			{
				if(ms-last_a_stim >900)
				{
					last_a_stim = ms;
					on_a_stim = ms + 300;
