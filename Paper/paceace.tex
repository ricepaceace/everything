...
and ventricular peaks will allow pacemakers to set their own % Anthropormophisizing pacemakers 
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\node[pin={[pin distance=3pt]320:{\VPH}}] at (m4) {};
			\legend{$f[n]$}
		\end{axis}
	\end{tikzpicture}
	\caption{
...
\figurename~\ref{fig:singlebeat} shows how these parameters correspond to portions of the CES.  %TODO: Not really sure what to say here.
...
\end{equation*}
Note that $w[n]$ is essentially $f'[n]^2  f[n]$, but with the sign adjusted to match the sign of $f'[n]$. %???: Does using the derivative notation make this more clear?
...
be the centers of the ventricular and atrial clusters %TODO: (Yamin:I like centers here, as I think it is more intuitive for the reader)
...
such that $beats(th_i)$ is biologically infeasible. %TODO: (Yamin: We used infeasible in the documentation we turned in, and I think its fine)
...
i.e. the number of $n$ such that $beats[n] = 1, beats[n-1] = 0$. % ???: Discuss the adjustment for extra long beats?
...
Our detection error rates are appropriately low across % ???: What does appropriately low mean?
...
for all 51 channels and \APW{} and \APH{} correctly for 50 of % ???: If most of the FN error is from ventricular, what about the atrial FN?
the channels.
...
our algorithm described in Section IV, we interfaced our % connected instead of interfaced?
...
a rat, and is kept beating by perfusing it with a special solution. % ??? Does special solution sound too dumb?
